...
**A skip branch that does not increment the suite's own skip tally renders identically to a section that ran and found nothing wrong.** `tests/test-hook-tmpfile.sh` section C's skip branch does not set `SKIPPED_SECTIONS=1` the way section A's does, so a skipped section contributes zero PASS and zero FAIL and the suite still exits 0 — a green guard and an absent one are the same exit code. This was cosmetic before #371: `pre-tool-hook.sh`'s own comment names section C as the sole exerciser of the `awk -f <(printf ...)` fallback path, so the one automated exercise of that path can go to zero assertions and say nothing (#380). If you add a skip branch to a section that is the sole coverage for something, set the same skip-tally flag every other skip branch in the file already sets — a shape one section already gets right, sitting next to the shape that does not.

**An `assert_*` helper billed as a suite's own discriminating check must not be satisfiable by an environment gap the suite claims not to have.** `assert_single_valid_json()` — the helper `tests/test-uncaptured-awk-crash-403.sh`, `tests/test-awk-crash-397.sh` and `tests/test-jit-match.sh` all call the check that catches a double-printed JSON object — silently no-ops on a host with no `python3`: it prints `SKIP` and returns without touching `$PASS`/`$FAIL`, so a `python3`-less host reports a clean pass while the one assertion the suite is actually about never ran, with nothing in the exit code to tell that apart from a genuine pass (#403). The same file's own SIGSEGV-shimming and chmod-555-unwritability preconditions get this right — a loud, whole-suite `exit 2` — so that is the shape to copy rather than a silent per-assertion no-op. Whether `python3` (not `python`) is actually on the `windows-latest` runner `tests.yml`'s `hooks` matrix job uses was not confirmed as of #403; if it is missing there, this degrade is live on every CI run using this helper.
...
**Concurrent lanes on one clone can crash a suite's own `awk`, not just slow it down (#393).** #388 and #389 each independently reported `tests/test-stop-hook.sh`, `tests/test-awk-locale-pins.sh` or `tests/test-entry-bytes.sh` failing, in a file the branch under test never touched, non-reproducing, a different section red each time — while several lanes and review agents ran at once on one clone. Reproduced deliberately: the same three suites, run concurrently across four worktrees of one clone, 60 rounds total. `test-stop-hook.sh` failed on 5–8 of every 15–20 rounds, always a different section; `test-awk-locale-pins.sh` and `test-entry-bytes.sh` never failed once across the same 60 rounds.

Audited first for the shared-state channel #388 found by accident (`trap.d/388.git-stash-shared-across-worktrees.md`) and found none here: all three suites build fixtures under their own `mktemp -d`, none call `git stash`/`checkout`/`commit`, none write to a fixed path under `$TMPDIR`. `test-entry-bytes.sh` sets `CLAUDE_PROJECT_DIR` to the worktree tree itself, rather than a fixture, at three call sites -- but only to source `common.sh` for its function definitions in a subshell, and calls no function there that writes.

Instrumented next, with a debug file private to each fixture's own `mktemp -d` — a debug log shared across the concurrent invocations trying to prove this lost its own writes under the same load, which is a trap worth not repeating, not a finding about the hook. That instrumentation caught it live: `stop-hook.sh`'s JSON-parsing `awk` call, ordinarily piped `2> /dev/null`, exited **139 — SIGSEGV** on eight separate invocations spread across eight different sections, all under the same concurrent load. `stop-hook.sh` already treats a failed awk parse as "unknown" and stays silent (#284's own deliberate contract, for a genuinely broken `awk` on a stranger's machine) — which is exactly why the assertion then fails: the report the fixture expects never gets built, and whichever section happens to run during the crash window is the one that goes red.

CI is not exposed to this as configured today: `run-all.sh` runs every suite sequentially within one job, and `tests.yml` gives each platform/shard its own runner and its own clone — nothing schedules two suites, let alone two worktrees, concurrently on one CI machine. This is a contributor-path hazard, not a CI gap.

The unwritten assumption these suites make: not competing with several other `awk`-forking suites for CPU/memory at once. Running two or more suites, or worktrees, concurrently on one machine is expected to intermittently starve or crash a subprocess a hook forks, with no way for the suite to tell that apart from a real defect in the code under test. A `tests/test-stop-hook.sh` failure with no nearby code change, seen while other lanes are running: re-run it alone before trusting it.
