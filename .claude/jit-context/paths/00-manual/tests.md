...
Audited first for the shared-state channel #388 found by accident (`trap.d/388.git-stash-shared-across-worktrees.md`) and found none here: all three suites build fixtures under their own `mktemp -d`, none call `git stash`/`checkout`/`commit`, none write to a fixed path under `$TMPDIR`, and the two spots `test-entry-bytes.sh` sets `CLAUDE_PROJECT_DIR` to the worktree tree itself rather than a fixture only source `common.sh` for its function definitions and call no function that writes.
