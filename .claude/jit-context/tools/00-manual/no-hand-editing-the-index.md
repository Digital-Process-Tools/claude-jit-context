...
The `match` is anchored on the whole path and on the file name as a complete component, because it used to be the bare substring `00-index.tsv` — which also refused `docs/00-index.tsv.bak`, a backup nothing generates and nobody is forbidden to edit (#92). A sibling rule, `no-shell-writes-to-the-index.md`, covers the same write arriving through `Bash`.
