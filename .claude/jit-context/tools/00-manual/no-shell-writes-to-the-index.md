...
So this rule does not try. It names the three write forms that actually get typed — a `>`/`>>` redirect, `tee`, and an in-place `sed`/`perl` — and refuses those. It is a guard against what everyone does, not a sandbox, and the difference is deliberate: the alternative is a rule anchored on the file name, which refuses `cat`, `grep` and `echo see 00-index.tsv` along with them. #76 and #79 are what a block firing on commands nobody wrote it for costs; #92 is what the missing half costs.
