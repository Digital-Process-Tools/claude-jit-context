...
Tools is the only dimension that can `block`. The other two only inject. Each vocabulary/path entry, and any tools rule with no `mode:`, fires at most once per session; the marker is keyed on the payload's `session_id` and lives in `.claude/jit-context/.discovery/state/`, and `session-start-hook.sh` clears this session's and ages out the rest. A payload with no `session_id` — a hand-run hook, every test suite here — keeps no marker and therefore never dedups. A tools rule with `mode: once` is the one exception (#394): its marker is keyed on the payload's `transcript_path` basename instead, so a session and every agent it spawns get separate budgets rather than sharing the session-wide one — a spawn inherits its parent's `session_id` but carries its own `transcript_path`. See `docs/writing-entries.md`'s `mode` table for the reasoning.
