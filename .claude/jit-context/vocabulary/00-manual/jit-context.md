...
Tools is the only dimension that can `block`. The other two only inject. Every vocabulary/path entry fires at most once per session, unconditionally; the marker is keyed on the payload's `session_id` and lives in `.claude/jit-context/.discovery/state/`, and `session-start-hook.sh` clears this session's and ages out the rest. A payload with no `session_id` — a hand-run hook, every test suite here — keeps no marker and therefore never dedups. A tools rule has no such default: its own `mode:` decides. `remind` (the default when `mode:` is absent) fires on every matching call, never deduped. `mode: once` is the one tools mode that dedups — and its marker is keyed on the payload's `transcript_path` basename rather than `session_id` (#394), so a session and every agent it spawns get separate budgets instead of sharing one: a spawn inherits its parent's `session_id` but carries its own `transcript_path`. A payload with no usable `transcript_path` falls back to `session_id` instead (#398), the same session-wide dedup `once` always had, rather than firing on every call with an unbounded marker. See `docs/writing-entries.md`'s `mode` table for the reasoning.
