...
**That "not decidable" is correct for a script standing alone in the project directory, and it is not the end of the story (#189).** A hook process DOES know which copy served it, in the one moment it is running: `SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"`, at the top of every hook, already resolves to that invocation's own plugin root. Nothing today writes that fact anywhere durable, so `jit-doctor.sh`, run cold after the fact, still cannot answer it — which is the right verdict for what it is asked, not a gap in what it computes. Whether a hook should start leaving that trace in `hooks.log` (candidate 3 in #189's own three-way split) is a separate, larger change — it touches every hook and `common.sh`, not just the diagnostic — and is not done by this repository as of `0.5.0`. Do not read `cannot tell` / `not decidable from here` as something doctor merely has not gotten around to fixing; it is the honest answer to the question it is actually able to ask.
...
**This rule is load-bearing for a test, not just a style preference.** `tests/test-arg-flag-values.sh`'s `classify_script()` only knows how to read a bash argument-loop shape, and today that is fine because every tracked file under `scripts/` happens to be bash. This rule already permits awk and perl too, and a genuine awk or perl tool would land in that same "cannot classify" bucket the moment one is added — the classifier is coupled to a narrower thing than this rule (bash specifically, not bash/awk/perl), and it says so where the coupling lives (#193). If you widen this rule further, or add the first awk or perl script, go teach `classify_script()` that shape rather than letting the new red (`not-bash-script`) or a silent `no-flags` stand in for it.
...
`tests/test-line-citations.sh` **enforces** this over every tracked shell file under `scripts/` or `tests/` **and every tracked markdown file** — entries, examples, templates, changelog fragments, this file and the README alike. It **reports without failing** over the generated indexes, the json and yml, and the vendored `.oss/` tree, because a finding in any of those is fixed somewhere else: an index is rewritten by `rebuild-tsv.sh` from the entry, and `.oss/` is rewritten by the next `/oss:scaffold --apply`. `CHANGELOG.md` is **not swept** at all — it is assembled rather than edited, so a finding there is unactionable by construction. The needle is a tracked basename followed by a colon and a digit, and its false-positive surface was measured three times: `98386f1` gave 96 basenames, 10 hits, 0 false; `5b46095` gave 98, 12 and 0; `f3a3228` gave 100 basenames and one hit across the whole markdown surface, which was the real citation #198 fixed. Counts are pinned to a commit rather than written in the present tense, because both grow with every file added; the suite prints the live counts when it runs, and re-pinning on a rebase is the point rather than an inconvenience.

**Markdown was advisory until #198, and the scope of the move was the open question rather than the move itself.** It became *every* markdown file rather than the `.claude/jit-context/` entries alone: the rule above is unconditional, so enforcing it on one directory while merely reporting it on every other markdown file in the tree rebuilds the same half-closed shape one level in — and `examples/` and `templates/` are the shape a stranger copies into their own repo, which makes a rotted citation there worse than one in a dogfood entry rather than better. It binds this repository's contributor path only; an installed plugin cache holds a copy of `tests/` but never runs it. The one residual (`grep -n` output quoted verbatim in prose) is likelier here than in a shell comment, and the escape hatch is the same: write it as prose. **The selectors have their own control** — planted paths, positive and negative in one table — because a markdown file quietly classified advisory would print its findings and pass forever, and today's tree cannot tell that apart from a working sweep.
