...
The advisory half is advisory because `.claude/jit-context/paths/00-manual/tooling.md` was held by PR #192 when the check was written, and reddening a file you may not edit is how a check gets disabled in its first week. #192 has since landed; widening is one `awk` pattern in that suite plus one line in that entry.
