...
[![Version](https://img.shields.io/badge/version-0.10.0-orange)](.claude-plugin/plugin.json)
...
`run-all.sh` also times each suite and prints the times after the pass/fail tally, slowest first, plus a total — wall-clock seconds via bash's own `$SECONDS`, never `date +%s.%N`, which is a GNU extension BSD `date` on macOS does not support. This never changes the exit code and never fails a slow suite; it exists so the three CI legs are directly comparable in the log, since nobody here can measure Windows locally. If timing genuinely cannot be taken on some run, a suite line reads `n/a` rather than `0s` -- a suite that took no measurable time and one nobody could time must not render the same way.
