- **The hook own joined `additionalContext` stream can no longer be forged into a
  ...
  **Compatibility:** the raw shape of `additionalContext` changes for any consumer that
  ...
  `jit-dry-run.sh` was already checked and does not parse `\n---\n` itself. A downstream
  tool that hand-parsed `pre-prompt-hook.sh`'s raw output before this change should either
  read the new manifest line or skip past it (it always ends at the first `\n`).
