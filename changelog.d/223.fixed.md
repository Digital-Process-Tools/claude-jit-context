- **`jit-dry-run.sh`'s `report_hook()` can no longer be tricked into reporting a
  ...
  needed. `jit_decode_u00()` now only decodes a value in the 0..31 range the encoder can
  actually produce; anything else is left as the literal bytes it always was. This bug
  predates #223 — it shipped with #219's original `jit_decode_u00()` — and is fixed here,
  in the one place both consumers now share, rather than filed separately.
