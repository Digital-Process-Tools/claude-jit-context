- **`jit-init.sh`: a fresh install now has something to match** (#81). Driven on a bare
  ...
  **The keywords are the whole design of the entry.** It keys on `jit entry`,
  `jit-context`, `00-manual`, `rebuild-tsv` and `frontmatter keywords` — never on
  `vocabulary`, which people type about tokenisers, i18n and linguistics. `tests/test-jit-init.sh`
