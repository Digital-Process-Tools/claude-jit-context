...
Two prompts are **the same miss** when they share a content word — a token of three or more characters that is not a stopword and not an ordinary English or French word — after the same normalisation the prompt hook applies before looking a keyword up. So `xsd validation` and `validate the xsd` group on `xsd`; `validation` and `validate` do not, because nothing here stems. There is no similarity metric and no threshold to tune, and every prompt behind a row is printed under it, so you can always see why two merged and disagree with the grouping.
...
Ordinary words are set aside against `data/generic-words.txt`, the same list `rebuild-tsv.sh` consults to mark a keyword generic (#232, #386): `click`, `ready` and `update` recur on any real log and none of them is an entry. The header says how many were set aside, or that the list was opted out (`--generic-words ""`, or `JIT_CONTEXT_GENERIC_WORDS` set to empty), or that the list named could not be read — three states, because a report that filtered and one that could not must never print alike.

`--log PATH` reads a log elsewhere, `--min N` changes how many misses make a recurrence (default 2), `--top N` caps the list, `--generic-words PATH` swaps the wordlist, `--help` carries the grouping rule. The header always names the log's own byte size, and `--size-threshold N` (default 10000000, 10MB) names it again in its own sentence once the log has reached that size.
...
**`session-start-hook.sh` runs this for you now, and its own call is bounded (`--tail 5000`) — the one caller here that never chose the moment.** #233 part 3 made it synchronous on every session; #248 found that the read was unbounded and its cost linear in a file nothing rotates, measured at 0.136s over a 3.2MB log and 2.336s at 20x that size on the maintainer's machine. The automatic path reads only the log's most recent 5000 lines. What it puts in front of you is one line, one report, one action (#386): `JIT : you use these words a lot and no entry matches them: "preprod" x5, "rector" x3. Write one with /claude-jit-context:vocabulary <word>, or turn this off with JIT_CONTEXT_MISSES=off in .claude/jit-context/config.env`. Each item is one token, the same unit `jit-misses.sh` ranks — a shared *phrase* shows up as its separate words, each with its own count. Ordinary words are already filtered out, the same way the manual run filters them. A quiet project, or one with no log yet or an empty one, opens as it always has: silence. A log that could not be read or made sense of — unreadable, the wrong format, records but none of them from a prompt — says so instead: `JIT : recurring words could not be evaluated (<reason>)`, so that case stops rendering identically to a project with nothing recurring. A log at or past the size watch gets a line of its own, `JIT : hooks.log is 10.1 MB. Delete or rotate it: <path>`, beside the words line when both hold and alone when only it does. Rotating the log itself is a larger change and is not done here.
...
**A fourth state sits ahead of all three above it: the harness re-invoking Stop over the very report this hook just printed.** A hook whose additionalContext blocks a turn from ending gets re-run with `stop_hook_active` set on the payload, and a hook that never reads that field cannot tell the re-entry from a first stop -- it reprints the identical numbered list, which blocks the turn again, which triggers another re-entry. `stop-hook.sh` reads `stop_hook_active` in the same parse pass that already yields the session key and, when it is true, answers with the same empty `{}` the missing-tree case uses -- before the marker-reading logic above ever runs, regardless of what fired or whether anything was edited.

**None of the four states above reach the model at all unless the project has asked for them.** #291 and #295 found that this report's real audience is a human curating `.claude/jit-context/`, not a model mid-session — a completed subagent once woke three times to answer the "none updated" line as though it were a task. `JIT_CONTEXT_STOP_REPORT` (`docs/configuration.md`) gates the whole thing, off by default: every shape above collapses to `{}` unless it is set to `1`. `hooks.log` keeps getting the same detail regardless, on every session, flag or no flag — this setting only decides whether a copy of it also reaches the model's own turn.
