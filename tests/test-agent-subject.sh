...
if grep -q "^Agent$TAB" "$BASE/tools/00-manual/00-index.tsv"; then
  PASS=$((PASS + 1))
  echo "  PASS: A rebuild-tsv.sh indexed the tool: Agent row"
else
  FAIL=$((FAIL + 1))
  echo "  FAIL: A no Agent row in the index -- the fixture is wrong, not the hook"
fi
...
OUT=$(run_agent "$PROJ" "explore" "go and look at the billing code")
RC=$?
...
OUT=$(run_agent "$PROJ" "general-purpose" "do something")
RC=$?
...
if [ "$OUT" = "{}" ]; then
  PASS=$((PASS + 1))
  echo "  PASS: E a subject-less dispatch with no rule naming it is still {}"
else
  FAIL=$((FAIL + 1))
  echo "  FAIL: E a subject-less dispatch with no rule naming it is still {}"
  echo "    got: ${OUT:-<EMPTY>}"
fi
...
if [ "$OUT" = "{}" ]; then
  PASS=$((PASS + 1))
  echo "  PASS: H and the call is silent, exactly as it was before #182"
else
  FAIL=$((FAIL + 1))
  echo "  FAIL: H and the call is silent, exactly as it was before #182"
  echo "    got: ${OUT:-<EMPTY>}"
fi
