...
#
# The first three bind SHELL_RULE rather than using assert_allows, for the same reason the
# Edit/Write block above binds EDIT_RULE (#239):
# .claude/jit-context/tools/01-oss/raw-file-reads-are-uncapped.md -- generated by the `oss`
# plugin's scaffold, replaced wholesale on every install, not ours to edit or narrow --
# refuses a bare cat/grep/`sed -n` at command position for its own unrelated reason ("go
# through supertool's capped read"), whatever file is named. That is a real, independent
# rule doing its own job; a plain assert_allows could not tell the two refusals apart, so on
# a tree carrying that layer this suite reported 3 failures for a rule that was never at
# fault (discovered when /oss:scaffold --apply first installed it here).
assert_not_blocked_by "cat on the index" "$(bash_payload "cat $IDX")" "$SHELL_RULE"
assert_not_blocked_by "grep on the index" "$(bash_payload "grep foo $IDX")" "$SHELL_RULE"
# Paired with "sed -i on the index" above: same command word, same path, one flag apart.
assert_not_blocked_by "sed reading, not writing" "$(bash_payload "sed -n 1p $IDX")" "$SHELL_RULE"
