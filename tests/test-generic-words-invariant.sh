...
# but the comparison itself is still an exact, case-sensitive, unfolded match against
# this file's own bytes), but never folds the file itself. A row stored with an accent, or with anything outside
