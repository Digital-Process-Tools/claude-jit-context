...
PASS=$((PASS + 1)); echo "  PASS: a healthy tree exits 0"
...
ST=0; run_doctor --base "$CACHEPROJ/.claude/jit-context" || ST=$?
...
ST=0; run_doctor --base "$HOOKPROJ/.claude/jit-context" || ST=$?
...
ST=0; run_doctor --base "$QUIETPROJ/.claude/jit-context" || ST=$?
...
ENV
ST=0; run_doctor --base "$CFGPROJ" || ST=$?
...
ST=0; run_doctor --base "$TYPOPROJ" || ST=$?
...
ST=0; run_doctor --base "$JUNKPROJ" || ST=$?
...
ST=0; run_doctor --base "$STALEPROJ" || ST=$?
...
ST=0; run_doctor --base "$LOGPROJ" || ST=$?
...
ST=0; run_doctor --base "$ADVPROJ" || ST=$?
...
for cand in C.UTF-8 en_US.UTF-8 en_US.utf8; do
  ...
  if [ "$n" = 2 ]; then UTF8="$cand"; break; fi
done
if [ -z "$UTF8" ]; then
  ...
  if [ "$KWROWS" = 1 ]; then
    PASS=$((PASS + 1)); echo "  PASS: a 3-byte keyword is not flagged for being 2 characters"
  else
    FAIL=$((FAIL + 1)); echo "  FAIL: a 3-byte keyword is not flagged for being 2 characters"
