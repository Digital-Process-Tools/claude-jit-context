...
require_log() {
  ...
  if [ ! -f "$log" ]; then
    FAIL=$((FAIL + 1))
    echo "  FAIL: $what -- hooks.log was never written, so nothing below is evidence"
    return 1
  fi
  if [ ! -s "$log" ]; then
    FAIL=$((FAIL + 1))
    echo "  FAIL: $what -- hooks.log is empty, so nothing below is evidence"
    return 1
  fi
  LOGTEXT="$(cat "$log")"
  PASS=$((PASS + 1))
  echo "  PASS: $what -- the harness can see a log line"
  return 0
}
...
MISSES=$(CLAUDE_PROJECT_DIR="$PROJ" bash "$SCRIPTS/jit-misses.sh" --min 1 2>&1)
MRC=$?
assert_contains "D jit-misses reads the capped log as a hook log" "$MISSES" "prompt record(s)"
if [ "$MRC" = 2 ]; then
  FAIL=$((FAIL + 1))
  echo "  FAIL: D jit-misses could not evaluate the capped log"
  echo "    got: $MISSES"
else
  PASS=$((PASS + 1))
  echo "  PASS: D jit-misses did not go dark on the capped log"
fi
...
if require_log "F" "$BASE/.discovery/logs/hooks.log"; then
  ...
  if grep -qF "not listed" <<< "$LOGTEXT"; then
    FAIL=$((FAIL + 1))
    echo "  FAIL: F an ordinary line was marked as truncated"
    echo "    got: $LOGTEXT"
  else
    PASS=$((PASS + 1))
    echo "  PASS: F an ordinary line carries no truncation marker"
  fi
fi
