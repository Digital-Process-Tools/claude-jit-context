...
echo ""
echo "=== Vocab: a path token survives when \$HOME is unset too (found in #362 self-review) ==="
# The sibling of the #361 bug, on the OTHER variable at the same two lines: `project`
# always carries a byte from bash's own "${CLAUDE_PROJECT_DIR:-.}" fallback, but
# `home="$HOME"` has none. jit_re_lit("") correctly returns "" -- nothing to escape
# -- and an unguarded gsub("" "/", "", tt) degenerates to gsub("/", "", tt), deleting
# EVERY slash in tt rather than a leading prefix that was never there. Same
# corruption class, reached from the other side: `src/pipeline/config.yml` becomes
# `srcpipelineconfig.yml` and "pipeline" stops matching.
T361B_LOG="$TEST_DIR/.claude/jit-context/.discovery/logs/hooks.log"
: > "$T361B_LOG"
T361B_OUT=$(cd "$TEST_DIR" && env -u HOME CLAUDE_PROJECT_DIR="$TEST_DIR" bash "$HOOK" \
  <<< '{"tool_name":"Bash","tool_input":{"command":"./supertool read:src/pipeline/config.yml"}}' \
  2> /dev/null)
assert_contains "[#361] pipeline vocab still matches with no \$HOME either" "$T361B_OUT" "pipeline vocabulary"
assert_path_contains "[#361] the log tail keeps every slash with no \$HOME" "$T361B_LOG" "src/pipeline/config.yml"
