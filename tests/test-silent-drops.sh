...
TEST_DIR=$(mktemp -d) || {
  echo "  SKIPPED: no writable temp dir"
  exit 2
}
...
mk_tree "$CLEAN" "invoice, ledger" || {
  echo "  SKIPPED: could not build fixtures"
  exit 2
}
mk_tree "$DIRTY" "file, invoice" || {
  echo "  SKIPPED: could not build fixtures"
  exit 2
}
...
clean_out=$(CLAUDE_PROJECT_DIR="$CLEAN" bash "$REBUILD" 2>&1)
clean_rc=$?
dirty_out=$(CLAUDE_PROJECT_DIR="$DIRTY" bash "$REBUILD" 2>&1)
dirty_rc=$?
...
whole_out=$(CLAUDE_PROJECT_DIR="$WHOLE" bash "$REBUILD" 2>&1)
whole_rc=$?
broken_out=$(CLAUDE_PROJECT_DIR="$BROKEN" bash "$REBUILD" 2>&1)
broken_rc=$?
